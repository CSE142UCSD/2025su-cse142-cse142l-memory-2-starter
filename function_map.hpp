...
extern "C"
uint64_t **to_2d_array(uint64_t *data, uint64_t size, uint64_t arg1){
    uint64_t **matrix = new uint64_t*[size/arg1];
    uint64_t pointer = 0;
    for(uint64_t i = 0; i < size/arg1; i++, pointer+=arg1) {
        matrix[i] = &data[pointer];
    }
    return matrix;
}
